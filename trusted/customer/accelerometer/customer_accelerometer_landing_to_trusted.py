...
sanitized_customer = customer_landing.filter(customer_landing['sharewithresearchasofdate'].isNull())

# Select the desired columns from the sanitized customer data
customer_trusted = sanitized_customer.select('customerName', 'email', 'phone', 'birthDay', 'serialNumber', 'registrationDate', 'lastUpdateDate', 'shareWithResearchAsOfDate', 'shareWithPublicAsOfDate', 'shareWithFriendsAsOfDate')

# Filter the accelerometer data for customers who have agreed to share
sanitized_accelerometer = accelerometer_landing.join(sanitized_customer, accelerometer_landing['user'] == sanitized_customer['email'])
